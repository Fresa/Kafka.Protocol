    ...
    public class WriteShareGroupStateRequest : Message, IRespond<WriteShareGroupStateResponse>
    {
        public WriteShareGroupStateRequest(Int16 version)
        {
            if (version.InRange(MinVersion, MaxVersion) == false)
                throw new UnsupportedVersionException($"WriteShareGroupStateRequest does not support version {version}. Valid versions are: 0-1");
            Version = version;
        ...
        public class WriteStateData : ISerialize
        {
            ...
            public class PartitionData : ISerialize
            {
                ...
                int ISerialize.GetSize(bool asCompact) => GetSize(asCompact);
                internal int GetSize(bool _) => _partition.GetSize(IsFlexibleVersion) + _stateEpoch.GetSize(IsFlexibleVersion) + _leaderEpoch.GetSize(IsFlexibleVersion) + _startOffset.GetSize(IsFlexibleVersion) + (Version >= 1 ? _deliveryCompleteCount.GetSize(IsFlexibleVersion) : 0) + _stateBatchesCollection.GetSize(IsFlexibleVersion) + (IsFlexibleVersion ? CreateTagSection().GetSize() : 0);
                ...
                /// <para>Default: -1</para>
                /// </summary>
                public PartitionData WithDeliveryCompleteCount(Int32 deliveryCompleteCount)
                {
