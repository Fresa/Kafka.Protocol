    ...
    public class ConsumerGroupHeartbeatRequest : Message, IRespond<ConsumerGroupHeartbeatResponse>
    {
        public ConsumerGroupHeartbeatRequest(Int16 version)
        {
            if (version.InRange(MinVersion, MaxVersion) == false)
                throw new UnsupportedVersionException($"ConsumerGroupHeartbeatRequest does not support version {version}. Valid versions are: 0-1");
            Version = version;
        ...
        internal override int GetSize() => _groupId.GetSize(IsFlexibleVersion) + _memberId.GetSize(IsFlexibleVersion) + _memberEpoch.GetSize(IsFlexibleVersion) + _instanceId.GetSize(IsFlexibleVersion) + _rackId.GetSize(IsFlexibleVersion) + _rebalanceTimeoutMs.GetSize(IsFlexibleVersion) + _subscribedTopicNamesCollection.GetSize(IsFlexibleVersion) + (Version >= 1 ? _subscribedTopicRegex.GetSize(IsFlexibleVersion) : 0) + _serverAssignor.GetSize(IsFlexibleVersion) + _topicPartitionsCollection.GetSize(IsFlexibleVersion) + (IsFlexibleVersion ? CreateTagSection().GetSize() : 0);
        ...
        /// <summary>
        /// <para>The member id generated by the consumer. The member id must be kept during the entire lifetime of the consumer process.</para>
        /// <para>Versions: 0+</para>
        /// </summary>
        ...
        public String? SubscribedTopicRegex
        {
            ...
            {
                if (Version >= 1 == false)
                    throw new UnsupportedVersionException($"SubscribedTopicRegex does not support version {Version} and has been defined as not ignorable. Supported versions: 1+");
                if (Version >= 1 == false && value == null)
                    throw new UnsupportedVersionException($"SubscribedTopicRegex does not support null for version {Version}. Supported versions for null value: 1+");
