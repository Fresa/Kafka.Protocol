    ...
    public class OffsetFetchResponse : Message
    {
        public OffsetFetchResponse(Int16 version)
        {
            if (version.InRange(MinVersion, MaxVersion) == false)
                throw new UnsupportedVersionException($"OffsetFetchResponse does not support version {version}. Valid versions are: 1-10");
            Version = version;
        ...
        public class OffsetFetchResponseGroup : ISerialize
        {
            ...
            public class OffsetFetchResponseTopics : ISerialize
            {
                ...
                int ISerialize.GetSize(bool asCompact) => GetSize(asCompact);
                internal int GetSize(bool _) => (Version >= 8 && Version <= 9 ? _name.GetSize(IsFlexibleVersion) : 0) + (Version >= 10 ? _topicId.GetSize(IsFlexibleVersion) : 0) + (Version >= 8 ? _partitionsCollection.GetSize(IsFlexibleVersion) : 0) + (IsFlexibleVersion ? CreateTagSection().GetSize() : 0);
                ...
                /// <para>Versions: 10+</para>
                /// </summary>
                public OffsetFetchResponseTopics WithTopicId(Uuid topicId)
                {
                    TopicId = topicId;
                    return this;
                }
